...
import AccurateInterval, { AccurateIntervalError } from "../src/scripts/Timer/AccurateInterval.mjs";
...
describe('AccurateInterval starting and stopping behavior', () => {
    ...
    describe('Prevent improper starts and stops', () => {
        ...
        it('Prevent starting twice', () => {
            intervalController.start();
            expect(intervalController.start.bind(intervalController)).toThrowError(
                AccurateIntervalError
            );
            intervalController.stop();
        });

        it ('Prevent stopping when not yet started', () => {
            expect(intervalController.stop.bind(intervalController)).toThrowError(
                AccurateIntervalError
            );
        });
    });
...
describe('AccurateInterval ticking behavior', () => {
    ...
    describe('With drift', () => {
        ...
        class PausableController extends AccurateInterval {
            /**
             * Creates an accurate interval with a pausable controller
             * @param  {ConstructorParameters<typeof AccurateInterval>} args 
             */
            constructor(...args) {
                super(...args);
            }
        ...
        it('Single drift and catch-up', () => {
            ...
            let running_count = phase / interval - 1;

            expect(spy).toHaveBeenCalledTimes(running_count);
            ...
            expect(spy).toBeCalledTimes(running_count);
        });

        it('Predictive drift', () => {
            // Set up while running
            jest.advanceTimersByTime(phase);
            let running_count = phase / interval;
